...
from combo_selector.app_profile import APP_PROFILES, resolve_app_profile
...
class ComboSelectorMain(CustomMainWindow):
    ...
    Signal Flow:
        ...
        3. metric_computed → orthogonality_metric_computed()
           ...
           → _on_redundancy_finished():
               • update_correlation_group_table() (signals blocked)
               • results page initialized
               • export page initialized
               • compute_custom_orthogonality_metric_score()
               → UpdateTableResultsWorker (updates model + table in background)
        4. exp_peak_capacities_loaded → update_results_with_new_exp_peak_capacities()
    """

    def __init__(self, app_profile=None):
        ...
        """
        super().__init__()
        self.app_profile = resolve_app_profile(
            getattr(app_profile, "key", app_profile)
        )
        ...
        self.results_page = ResultsPage(self.model, app_profile=self.app_profile)
        ...
        if self.app_profile.show_pairwise_page:
            self.add_side_bar_item(
                "Pairwise \n2D Combinations",
                self.plot_page,
        ...
        if self.app_profile.show_redundancy_page:
            self.add_side_bar_item(
                "Metric Correlation\nAnalysis",
        ...
        if self.app_profile.show_export_page:
            self.add_side_bar_item(
        ...
        self.set_status_text(f"{self.app_profile.display_name} mode loaded")
    ...
    def _on_redundancy_finished(self):
        ...
        Executed in strict order so that the results page is only
        initialized after the correlation groups exist, and score
        computation only starts once the page is fully set up:

        1. Hide the OM calculation progress overlay.
        2. Build correlation groups (signals blocked so the auto-connected
           ``correlation_group_ready`` signal does not fire prematurely).
        3. Initialize the results page UI — safe now that groups are ready.
        4. Initialize the export page.
        5. Explicitly trigger score computation on the results page.
        """
    ...
    def update_results_with_new_exp_peak_capacities(self) -> None:
        ...
        self.results_page.update_results_table_view()
...
def _parse_launch_configuration(argv=None, default_profile: str | None = None):
    """Parse launch arguments while preserving Qt-compatible arguments."""
    parser = argparse.ArgumentParser(add_help=True)
    parser.add_argument(
        "--profile",
        choices=sorted(APP_PROFILES),
        default=default_profile or os.getenv("COMBO_SELECTOR_PROFILE", "advanced"),
    )
...
def main(argv=None, default_profile: str | None = None):
    ...
    args, qt_argv = _parse_launch_configuration(
        argv=argv, default_profile=default_profile
    )
    ...
    window = ComboSelectorMain(app_profile=args.profile)
    window.showMaximized()
    sys.exit(app.exec())


def main_advanced():
    """Launch the full-featured advanced application profile."""
    main(default_profile="advanced")


def main_user():
    """Launch the end-user application profile with essential features only."""
    main(default_profile="user")
