...
class MetricOptimizationTests(unittest.TestCase):
    ...
    # %FIT relies on numerical optimization whose convergence can vary slightly
    # between supported SciPy versions. A 0.001 absolute tolerance corresponds
    # to less than 0.2% across the regression values and is scientifically
    # negligible for this metric. Other metrics retain a strict 1e-12 tolerance.
    ...
    def test_all_optimized_metrics_match_baseline_values_and_ranks(self) -> None:
        ...
        for metric, expected_values in self.BASELINE_VALUES.items():
            ...
            # Apply the documented SciPy tolerance only to %FIT; keep all other
            # optimized metrics constrained to their original numerical values.
            tolerance = self.METRIC_ABSOLUTE_TOLERANCES.get(metric, 1e-12)
