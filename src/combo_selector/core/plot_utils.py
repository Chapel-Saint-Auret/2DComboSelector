...
class PlotUtils:
    ...
    def plot_modeling_approach(
        self, set_number: str = "", erase_previous: bool = True, draw=True
    ) -> None:
        """
    ...
    def plot_asterisk(self, set_number: str = "") -> None:
        ...
        self.axe.plot(
            time, time, label=f"$Z_-$: {Zminus:.3f}", color="black", linestyle="-"
        )[0]
        self.axe.plot(
            time, 1 - time, label=f"$Z_+$: {Zplus:.3f}", color="black", linestyle="--"
        )[0]
        self.axe.vlines(
